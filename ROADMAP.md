...
- Bundled profiles across 3 engine types (`rancher`, `suse-virt`, `suse-edge`): `rancher` (1 VM), `test` (2-node), `harvester-ha` (3-node HA), `harvester-2n` (2-node + Rancher), `harvester` (3-node + Rancher; AWS via same profile + `--target aws`, live-validated 2026-09-11 on `m8id.8xlarge`), `suse-edge` (Rancher + Elemental + EIB + 4 edge nodes), plus workshop overlays `virt-workshop-aws` / `virt-workshop-aws-2n`. **Option A:** no separate `*-aws` topology twins — AWS is *where* (`deployment_target: aws` / `--target aws` + `instance_catalog`), not a second *what*.
...
**Sizing (Option A):** three tiers per **topology** profile (see `rodeo/providers/instance_catalog.py`); no separate `*-aws` catalog keys. **recommended** for `harvester` and `harvester-2n` is `m8id.8xlarge` (32 vCPU / 128 GiB / a single ~1.9 TiB NVMe device), avoiding `i7i.8xlarge`'s waste: same vCPU/RAM, but its NVMe is split across two ~3.4 TiB devices and rodeo only mounts one. `suse-edge` recommended is `m8id.4xlarge`. Explicit `provider.instance_type` still wins. Metal remains the performance escape hatch. Tiny types are rejected at validate. Guest disks default to a **flat 500 GB/Harvester-node, 60 GB/Rancher-node** floor under aws host-context (3 x 500 + 60 = 1560 GB fits with margin on m8id.8xlarge's ~1.9 TiB device). Availability is checked in-region before create; no silent downsize. Canonical smoke: `rodeo up --yes --profile harvester --target aws --instance-tier recommended`.
...
**Fixed 2026-09-16 (Option A regression):** deleting the standalone `suse-edge-aws` profile also deleted its `rancher_tls: secret` override and RAM headroom — with nothing replacing it, `rodeo up --profile suse-edge --target aws` silently fell back to the bare-metal default (`letsEncrypt`). That can never work through rodeo's own managed security group: `MANAGED_SG_PORTS` (`rodeo/providers/aws.py`) only opens 22/8443/30002 to the operator's own IP, so the ACME HTTP-01 challenge on port 80 can never complete, and even a valid cert would sit behind Traefik on 443 — also not open. Added a `suse-edge`-specific branch to `host_context.py`'s `_apply_aws()`: forces `rancher_tls.source` to `secret` (self-signed + NodePort 30002, which the managed SG does open) whenever it is still the untouched `letsEncrypt` default, and restores the deleted profile's RAM/disk headroom (`rancher` 12 GiB, `eib` 16 GiB/150 GB, `edge-node` 25 GB). Never overrides an explicit non-default choice or shrinks a larger plan value.

**Fixed 2026-09-17 (harvester_auto_import + self-signed Rancher):** `_import_harvester()` (`rodeo/engine/rancher/harvester.py`) wrote the `cluster-registration-url` Setting's `value` as a bare manifest URL string instead of the `{"url": ..., "insecureSkipTLSVerify": ...}` JSON object Harvester's setting-controller actually expects, and never set `insecureSkipTLSVerify` at all. Silent on bare-metal/Instruqt (real Let's Encrypt cert, no skip ever needed), but self-signed Rancher (AWS's `rancher_tls: secret`) has no trusted CA — Harvester's backend fetch of the manifest fails outright without the flag (confirmed live: `curl` to the manifest URL from the Harvester node works with `-k`, hangs/fails without it). Found live importing `harvester-1.8.2-validation` on AWS 2026-09-17: Rancher showed the cluster stuck on `Waiting for API to be available` indefinitely. Fix sets `insecureSkipTLSVerify: self.tls_source != "letsEncrypt"` and correctly JSON-encodes the value. Only affects deploys with `harvester_auto_import: true` (off by default — see `test_auto_import_defaults_off`); the manual Exercise 1 import flow (Harvester's own Settings UI) is a separate code path (Harvester's own web form) and is unaffected by this specific fix.

**Fixed 2026-09-18 (silent partial seed):** reviewing a live `virt-workshop-aws` deploy on a fresh AWS host, `custom_scripts` reported success (`✓ custom_scripts 0:00`) but did nothing — no image cache, no NFS export, no `webserver-prod`/`daily-batch-processor`. `custom/scripts/` on the seeded lab held only an unexplained `.gitkeep` (a filename that has never existed anywhere in this profile's git history) instead of its 3 real scripts. The installed package on that same host was verified correct (current `main`, all 3 scripts present with the right content), and re-running the identical `seed_lab()` call immediately after, on the same host, reproduced correctly — so the root cause was never pinned down (candidate: a first-write consistency hiccup on a freshly-restored EBS root volume). Rather than chase a non-reproducible one-off further, `seed_lab()` now runs `_verify_seed_copy()` after every copy: it re-checks every file actually written this call against its source by size and raises loudly on any mismatch, instead of silently shipping a partial lab. `stream_custom_scripts()` also now logs a line when `custom/scripts/` exists but holds nothing executable — previously a silent no-op indistinguishable from "this profile ships no custom scripts."
