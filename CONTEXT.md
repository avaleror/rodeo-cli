...
| `deploy [--from P] [--force] [--check] [--finalise] [--tui/--no-tui]` | Full pipeline. `--check` = preflight only (root, /dev/kvm, nested virt, RAM, disk, tools). |
| `clean [--yes]` | LibvirtDriver destroy/undefine (virsh fallback), disk glob deletes, network teardown, state reset. |
